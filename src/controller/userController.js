...
export default {
    ...
    async displayUsers(req, res, next) {
        ...
        if (firstName || lastName || email) {
            users = allUsers.filter(function (value) {
                // console.log(value.first_name.includes(firstName));
                return value.first_name.includes(firstName) &&
    ...
    async changePassword(req, res, next) {
        // console.log(req.user, req.body.newPassword);
        User.findOne({ email: req.body.email })
            .then((user) => {
                user.setPassword(req.body.new_password, (err, user) => {
                    if (err) return next(err);
                    user.save();
                    res.status(200).json({ message: 'Password changed successfully. For secure resons, after 4 secounds you will be logged out' });
                });
            })
    },
